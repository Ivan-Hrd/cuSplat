...
__device__ void quatToMatrix(const float w, const float x, const float y, const float z, float R[3][3]) {
...
__device__ COV3D buildCov3D(const Gaussian& g) {
    float R[3][3];
...
__device__ void buildJ(float X, float Y, float Z, float fx, float fy, float J[2][3]) {
...
__global__ void screenspaceGaussians(const raft::device_span<Gaussian> gaussians, const Camera camera, const raft::device_span<GaussianSplated> res) {
    ...
    const float (*W)[3] = camera.R; // world->camera
    ...
    float mid = 0.5f * (a + c);
    float diff = 0.5f * (a - c);
    float lambda_max = mid + sqrtf(diff*diff + b1*b1); // plus grande valeur propre

    gaussian.radius = ceilf(3.0f * sqrtf(lambda_max));
    ...
    for (int c = 0; c < 3; c++) {
        gaussian.color[c] = max(0.0f, min(1.0f, gaussian.color[c] + 0.5f));
    }
